...
import randomBirdId from '../utility/random'
import {selectPoints} from '../utility/selectPoints'
...
class App extends React.Component {
  state = {
    ...
    RandomBird: randomBirdId(1, 6),
    activeNext: false,
    totalClick: 1,
    totalPoint: 0,
    currentInfoBird: null,
    finish: false,
    questionMark: 'https://www.meme-arsenal.com/memes/15ef8d1ccbb4514e0a758c61e1623b2f.jpg',
  }

  startGame = () => {
    this.setState({
      currentData: this.state.mainPage,
      currentPage: 'mainPage',
      totalPoint: 0,
      finish: false,
      RandomBird: randomBirdId(1, 6),
  ...
  clickAnswer = (id, index) => {
    const clickedBird = this.state.currentData.filter((item) => item.id === id)
    const coppyCurrentData = [...this.state.currentData]

    if (this.state.RandomBird === id) {
      coppyCurrentData[index] = {...coppyCurrentData[index], success: true}
      this.setState({
        totalPoint: this.state.totalPoint + selectPoints(this.state.totalClick),
        activeNext: true,
        currentInfoBird: clickedBird[0],
        currentData: coppyCurrentData.map((item) => {
          return {
            ...item,
            clicked: true,
          }
      ...
      coppyCurrentData[index] = {...coppyCurrentData[index], error: true, clicked: true}
      this.setState({currentInfoBird: clickedBird[0], currentData: [...coppyCurrentData], totalClick: this.state.totalClick + 1})
    }
  }

  secondClickAnswer = (id) => {
    const clickedBird = this.state.currentData.filter((item) => item.id === id)
    this.setState({currentInfoBird: clickedBird[0]})
  }
  ...
  clickNext = () => {
    switch (this.state.currentPage) {
      case 'mainPage':
        this.setState({
          currentData: this.state.sparrowsPage,
          currentPage: 'sparrowsPage',
          RandomBird: randomBirdId(1, 6),
          currentInfoBird: null,
          activeNext: false,
          totalClick: 1,
      ...
      case 'sparrowsPage':
        this.setState({
          currentData: this.state.forestBirdsPage,
          currentPage: 'forestBirdsPage',
          RandomBird: randomBirdId(1, 6),
          currentInfoBird: null,
          activeNext: false,
          totalClick: 1,
      ...
      case 'forestBirdsPage':
        this.setState({
          currentData: this.state.songBirdsPage,
          currentPage: 'songBirdsPage',
          RandomBird: randomBirdId(1, 6),
          currentInfoBird: null,
          activeNext: false,
          totalClick: 1,
      ...
      case 'songBirdsPage':
        this.setState({
          currentData: this.state.predatorBirdsPage,
          currentPage: 'predatorBirdsPage',
          RandomBird: randomBirdId(1, 6),
          currentInfoBird: null,
          activeNext: false,
          totalClick: 1,
      ...
      case 'predatorBirdsPage':
        this.setState({
          currentData: this.state.seaBirdsPage,
          currentPage: 'seaBirdsPage',
          RandomBird: randomBirdId(1, 6),
          currentInfoBird: null,
          activeNext: false,
          totalClick: 1,
        ...
        break
      case 'seaBirdsPage':
        this.setState({currentData: this.state.congratilation, currentPage: 'mainPage', finish: true})
        break
      default:
        this.setState({currentData: this.state.mainPage, currentPage: 'mainPage'})
        break
    }
  }
    ...
    return (
      <div className="container">
        <MainContainer
          ...
          secondClickAnswer={this.secondClickAnswer}
